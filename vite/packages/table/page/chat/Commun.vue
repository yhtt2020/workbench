<template>
      ...
      <vue-custom-scrollbar :settings="settingsScroller" style="height: 100%;">
        <div class="box">
          <div class="w-full h-full content">
            <!-- 循环渲染多个 ComCard -->
            <ComCard
              v-for="(card, index) in comCards"
              :key="index"
              :cardData="card"
              @click="toggleCardSelection(index)"
              :style="{ backgroundColor: card.isSelected ? 'rgba(80,139,254,0.20)' : 'rgba(0,0,0,0.30)' }"
          ...
          <div class="w-full h-full -ml-8 detail" v-if="isShow">
            <ComCard>
              <template #top-right>
                <CloseCircleOutlined @click="close" class="text-base" />
              </template>
              <template #content>
                阿皮有话说：由于服务器的表现不稳定，还是影响到了用户使用我们的功能。也
              </template>
            </ComCard>
          </div>
        </div>
  ...
  const comCards = ref([
    {
      content: '6.3.3版本测试体验版，欢迎大家下载体验。主要对聊天功能进行了加强。如何加入团队？开启聊天功能，设置-》快捷开关-》启用聊天搜索加入群聊，给大家一个体验群ID：@TGS#2QXE7QDNI',
      isSelected: false,
    },
    {
      content: '各位工作台的小伙伴们大家好。由于近期我们的服务器受到一些来自印度？海外的不明IP攻击。主要是攻击了我们产品站www.apps.vip。',
      isSelected: false,
      options: {
    ...
    {
      content: '8月9日，近期开发内容日志：桌面市场、超级工具箱、待办.经历了连续的几天的服务器震荡。目前我们已经基本稳定了服务器的表现。接下来应该会更加稳定。阿皮有话说：由…',
      isSelected: false,
      options: {
  ...
  const toggleCardSelection = (index) => {
    ...
    // 切换当前卡片的选中状态
    comCards.value[index].isSelected = !comCards.value[index].isSelected;
