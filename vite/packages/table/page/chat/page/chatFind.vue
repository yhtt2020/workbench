<template>
  <div class="w-full flex items-center justify-center px-3">
    <template v-if="settings.isLoading !== 0">
      <a-spin />
    </template>
    ...
    <template v-else>
      <div class="flex flex-col">
        ...
        <div class="content-list">
          <div v-for="item in groupList" class="flex items-center content-item justify-between px-4 py-5 rounded-xl" style="background: var(--secondary-bg);width: 400px;">
...
export default {
  ...
  computed:{
    ...mapWritableState(chatStore,['groupList','refUser','settings','memberList','limitTotal']),
  ...
  async mounted(){
    ...
    await this.getMemeber()
  },
  methods:{
    ...mapActions(chatStore,['getReferData','getMemeber']),
    openUserCard(uid){
