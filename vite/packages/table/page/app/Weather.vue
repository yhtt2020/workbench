<template>
  <div id="display">
    <a-tabs
      v-model:activeKey="currentCity"
      type="editable-card"
      @edit="onEdit"
      :destroyInactiveTabPane="true"
    >
      <a-tab-pane
        v-for="city in appData.weather.cities"
        :key="city.id"
        :tab="city.name"
      >
            ...
            <div style="width: 40em; display: inline-block">
              <a-row style="">
                <a-col :span="8" style="font-size: 2.5em">
                  <i :class="'qi-' + city.weather.now.icon + '-fill'"></i>
                  {{ city.weather.now.text }}
                </a-col>
                <a-col :span="8">
                  <div style="font-size: 2em">
                    {{ city.weather.now.temp }} ℃
                  </div>
                  <div>风力 {{ city.weather.now.windScale }}级</div>
                </a-col>
                <a-col :span="8">
                  <Icon icon="position"></Icon> {{ city.adm2 }}-{{ city.name
                  }}<br />
                  <Icon style="margin-left: 2em" icon="shijian"></Icon> 发布于
                  {{ getObsTime(city.weather.now.obsTime) }}
                </a-col>
              </a-row>
            </div>
          </div>
            ...
            >
              <a-row style="width: 150em; margin-bottom: 0.7em">
                <a-col
                  ...
                  :span="1"
                  v-for="w in city.h24.hourly"
                >
                  {{ getdHours(w.fxTime) }}:00
                  <br />
                  <i
                    style="font-size: 1.2em"
                    :class="'qi-' + w.icon + '-fill'"
                  ></i>
                  {{ w.text }}
                  <br />
                  {{ w.temp }}℃
