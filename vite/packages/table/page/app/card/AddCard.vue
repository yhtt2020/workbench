...
export default {
  ...
  data () {
    return {
      cardList: [
        ...
        { name:"games", cname:"Steam折扣推荐",icon:"steam",detail:"热门折扣游戏推荐",images:['GamesDiscount','DiscountPercentage','GameEpic','MyGameSmall','MyGameMiddle','capture','audio','voice']},
        { name:"remote", cname:"外部卡片",icon:"game",detail:"添加一个链接到外部网页的卡片，可以实现各种自定义功能。",images:['Remote'],size:['1x2']},
  ...
  computed: {
    filterList () {
      return this.cardList.filter(i => {return i.cname.toLowerCase().includes(this.selectContent.toLowerCase()) || i.detail.toLowerCase().includes(this.selectContent.toLowerCase())})
    },
    count () {
      let i = 0
