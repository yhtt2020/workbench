<template>
  <HomeComponentSlot :customIndex="customIndex"  :options="options" :formulaBar="formulaBar" ref="customAssembly" :custom-data="customData">
    ...
    <div class="h-full w-full pointer "  @click="openApp" v-if="Object.keys(myData.img).length>0">
...
<template #extra>
  <div class="flex justify-center items-center rounded-lg h-10 drawer-item-bg w-16  pointer" @click="saveData">保存</div>
</template>
    <vue-custom-scrollbar   :settings="rightScrollbarSettings"
                           class="relative"
                           style="height: calc(100% - 10px);">
    <div class="text-base">小组件尺寸</div>
    <HorizontalPanel :navList="sizeList" v-model:select-type="mySize" class="mt-6" bg-color="drawer-item-select-bg" ></HorizontalPanel>
    <div class="flex flex-row justify-between items-center mt-6">
      <div class="text-base">显示名称</div>
      <div><a-switch v-model:checked="showName" /></div>
    </div>
    <a-input v-model:value="myData.title" class="rounded-lg mt-6 drawer-item-bg" style="height: 48px" placeholder="">
    </a-input>
    <div class=" mt-6 text-base">小组件封面</div>
<!--    <div class=" mt-2 text-base" style="color: rgba(255, 255, 255, 0.4);">支持直接复制粘贴图片到此处</div>-->
    <div class="flex flex-row justify-between items-center mt-6" v-if="!Object.keys(myData.img).length>0">
      <div class="flex justify-center items-center rounded-lg  h-12 drawer-item-bg w-1/2 pointer text-base" @click="importFile">选择图片</div>
      <div class="flex justify-center items-center rounded-lg h-12 drawer-item-bg w-1/2 ml-3 pointer text-base" @click="openMy">选自壁纸收藏</div>
    </div>
    <div v-else>
      <div class="h-16 w-16 mt-6 relative" >
        ...
        <img :src="myData.img.middleSrc"   alt="" class="h-full w-full" style="border-radius: 8px;object-fit: cover" v-else-if="myData.img.middleSrc">
        <img :src="myData.img.src"  class="h-full w-full" style="border-radius: 8px;object-fit: cover" v-else-if="myData.img.src">
        <div class="rounded-full  flex-shrink-0 w-5 h-5  flex justify-center items-center pointer absolute top-0 right-0 translate-x-1/2 -translate-y-1/2" @click="deleteImg" style="background: rgba(255, 255, 255, .5);">
          <Icon icon="guanbi" style="color:black;height: 10px"></Icon>
        </div>
      </div>
    </div>
    <div class=" mt-6 text-base">快捷方式</div>
    <div class=" mt-2 text-base" style="color: rgba(255, 255, 255, 0.4);">支持选择Windows应用的快捷方式来作为小组件的点击启动目标</div>
    <div class="flex flex-row justify-between items-center mt-6">
      <div class="flex justify-center items-center rounded-lg h-12 drawer-item-bg w-1/2 pointer" @click="customClick" v-if="!myData.link">本地应用</div>
      <div class="flex justify-center items-center rounded-lg h-12 drawer-item-bg w-1/2 ml-3 pointer"  v-if="!myData.link" @click="showAppNav">选自应用导航</div>
      <div v-else class=" w-full rounded-lg h-9 flex justify-between items-center px-2" style="background: rgba(42, 42, 42, 1);">
        <div class="text-more w-2/3">{{title}}</div>
        <div class="rounded-full  flex-shrink-0 w-5 h-5  flex justify-center items-center pointer" @click="deleteApp" style="background: rgba(255, 255, 255, .5);">
        <Icon icon="guanbi" style="color:black;height: 10px"></Icon>
         </div>
      </div>
    </div>
    ...
    <ModalList v-if="myImgShow" v-model:visible="myImgShow" title="我的收藏" :imgList="myPapers" @sendImg="sendImg"></ModalList>
...
export default {
  name: "CustomAssembly",
  components: {Template, HomeComponentSlot ,HorizontalPanel,Classification,listItem,ModalList},
  props:{
    customIndex:{
      type:Number,
      default:0
      ...
      if(typeof this.myData.link === 'object'&&this.myData.link.type){
        switch (this.myData.link.type) {
          ...
          case 'localApp':
            ...
            break
          case 'lightApp':
            ipc.send('executeAppByPackage', { package: this.myData.link.package })
            break
          default:
            require('electron').shell.openPath(this.myData.link.path)
        }
      }else if(this.myData.link){
        this.myData.link.path ? require('electron').shell.openPath(this.myData.link.path):
