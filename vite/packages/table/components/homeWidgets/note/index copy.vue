    ...
    <HomeComponentSlot :customData="customData" :customIndex="customIndex" :options="options" :formulaBar="formulaBar">
        <div ref="textRef" style="margin-top: 8px; " :style="{ width: w, height: h }">
            <textarea :style="{ 'backgroundImage': background }" class="box" placeholder="输入卡片内容" v-model="text"
                @blur="updateText" @click="test(121111)">
                </textarea>
            ...
            dragRef.addEventListener("mousedown", (el) => {
                ...
                document.onmousemove = function (e) {
                    ...
                    if (width > 250 && width < 542) {
                        ...
                        that.options.className = "card double"
                        console.log('盒子宽度在2x2范围内 :>> ');
                    }
                    else if (width < 250 && height > 140) {
                        if (height > 140 && height < 360) that.h = `${height}px`
                        that.options.className = "card"
                        console.log('盒子宽度在1x2范围内 :>> ');
                    }
                    else if (width < 250 && height < 140) {
                        that.options.className = "card small"
                ...
                document.onmouseup = function () {
                    ...
                    that.__updateSize(that.options.className)
                };
            }, { capture: true })
        },
        callBack(e) {
