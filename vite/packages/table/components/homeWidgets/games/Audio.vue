  ...
  <HomeComponentSlot :customData="customData" :customIndex="customIndex" :options="options">
    ...
    <div v-if="audioType.name === 'output'" class="mt-4 flex flex-col">
      ...
      <vue-custom-scrollbar  @touchstart.stop @touchmove.stop @touchend.stop :settings="settingsScroller" style="height:184px;">
        <div v-for="(item,index) in outputList" :class="outputIndex === index ? 's-item' :''" @click="selectOutputDevice(item,index)" class="w-full flex btn-active items-center rounded-lg justify-center pointer" style="padding: 8px 10px 6px 10px;color: rgba(255, 255, 255, 1);font-size: 14.64px;font-weight: 200;">
