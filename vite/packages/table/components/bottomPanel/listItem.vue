<template>
  <xt-task :modelValue="1" v-if="item.name == '主页'">
    ...
    <div class="m-3 flex flex-col items-start w-10/12">
      ...
      <div class="text-xs  text-more text-left  xt-text" style="width: calc(100%)" v-if="item.summary !== ''">{{ item.summary }}</div>
    </div>
  </div>
  </xt-task>
  <div v-else class="flex-row flex items-center  pointer h-16 p-3 w-full mb-2">
    <div class="h-10 w-10 rounded-lg flex justify-center items-center xt-bg xt-text" style="background:rgb(80, 80, 80);"
      v-if="item.type === 'systemApp'">
      <Icon style="font-size: 24px" :icon="item.icon" class="h-10 w-10"></Icon>
    </div>
