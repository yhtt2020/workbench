<template>
  ...
  <Widget :options="options" :customData="customData" :customIndex="customIndex" :desk="desk">
    ...
    <div class="w-full flex mt-3.5 mb-3">
      ...
      <div class="flex">
        <a-button type="primary" :disabled="isPrevAble" @click="prevButton" class="h-11 w-11 rounded-lg mr-3" :size="size">
          <template #icon>
            <LeftOutlined style="font-size: 1.5em;color: var(--active-text);"/>
          </template>
        </a-button>
        <a-button type="primary" :disabled="isNextAble" @click="nextButton" class="h-11 w-11 rounded-lg" :size="size">
          <template #icon>
    ...
    <template v-if="defaultStrategy.name === 'video'">
      <!-- 数据空状态 -->
      <div v-if="videoList.length === 0"  class="w-full flex items-center justify-center my-14">
    ...
    <template v-else>
      <!-- 数据空状态 -->
      <div v-if="graphicList.length === 0"  class="w-full flex items-center justify-center my-14">
        <a-empty :image="simpleImage" description="从工作台中启动游戏来查看相关攻略" />
      </div>
      
      <div v-else-if="textList.length === 0"  class="w-full flex items-center justify-center my-14">
        <a-empty :image="simpleImage" description="找不到相关游戏攻略" />
      </div>
           
      <!-- 图文攻略列表区域 -->
      <div v-else class="w-full h-full">
        <div v-for="item in textList" @click="openUrl(item.href)" class="p-3 mb-4 flex pointer items-center rounded-lg text-item-bg">
  ...
  <a-drawer v-model:visible="searchShow" title="搜索" width="500" placement="right">
    <div class="line">
      <a-input  class="no-drag h-10 w-full" 
        placeholder="搜索" spellcheck="false" v-model:value="searchWord"
