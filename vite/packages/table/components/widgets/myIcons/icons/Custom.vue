<template>
  <xt-zoom v-model:width="w" v-model:height="h" :style="[size]">
    <div class="xt-bg xt-shadow rounded-xl xt-text relative controller" :style="[size]">
      <XtBaseIcon @click="iconsRightClick()" icon="gengduo1" class="absolute icon" style="right: 8px; top: 4px">
      </XtBaseIcon>
      <div class="text-center cursor-pointer mx-auto truncate flex justify-center  items-center" style="height: 30px;" :style="[titleSize]"
        @click="titleVisible = true">
        {{ groupTitle }}
      </div>
      <div class="flex flex-wrap justify-between">
        <div v-for="i in count" @contextmenu.prevent.stop="rightClick(i - 1)">
          <Icon v-if="iconList[i - 1]" v-bind="iconList[i - 1]" :index="i - 1" :size="icons" :data-index="i - 1"
            :state="true">
          </Icon>
        </div>
        <div :style="[iconsState]" class="flex justify-center items-center flex-col cursor-pointer xt-hover rounded-xl"
          @click="fullScreenClick($event)" v-if="iconList.length > count">
          <XtIcon icon="gengduo1" w="38"></XtIcon>
          <div class="truncate w-full text-center text-xs" style="margin-top: 6px">
            更多
          </div>
        </div>
        <!-- 占位 -->
  ...
  <XtDrawer v-model="visible">
    <Set @editIcons="editIcons(index)" @deleteIcons="deleteIcons(index)" v-model:data="visible"></Set>
  </XtDrawer>
  <Teleport to="body">
    <xt-old-modal v-model="titleVisible" @close="titleVisible = false" title="修改分组名" :isFooter="false">
