<template>
 <Widget :desk="desk" :sizeList="sizeList" :options="options" :customIndex="customIndex" :menuList="menuList" ref="cardSlot">
  <template #left-title-icon>
   <div class="icon" style="width: 35px;height: 24px;display: flex; justify-content: center;align-items: center;position: absolute;left: 2px;">
  ...
  <template v-else>
    <div v-if="showWay">
     ...
     <MinCourierItem v-else :courier="courierDetailList[0]" @click="viewDeliveryDetails(this.deliveryDetails[0])"></MinCourierItem>
    </div>
    <template v-else>
     ...
     <template v-else>
      ...
      @click="viewDeliveryDetails(item)" /></vue-custom-scrollbar>
