    ...
    <Widget @click="onHistoryMessage" :sizeList="sizeList" :customData="customData" :customIndex="customIndex" :options="options" ref="cardSlot" :desk="desk"  :menuList="menuList">
        ...
        <div class="dash-board overflow-hidden" style="height: 370px;">
            <div class="dash-cell pointer" v-for="(item,index) in targetKeys" :key="index">
                ...
                <div class="cell-num">{{ this.mockData[item-1].num == 0?'-':this.mockData[item].num }}</div>
            </div>
        </div>

        <!-- 设置面板 -->
        <a-drawer :width="500" title="设置" v-model:visible="setVisible" placement="right">
            <template #extra>
...
export default {
    ...
    computed: {
        ...mapWritableState(shortTalkStore, ['mockData','access_token','access_token','baseUrl','setVisible']),
    },
    data() {
        return {
            sizeList: [
