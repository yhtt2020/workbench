<template>
  ...
  <div class="absolute inset-0" style="border-radius: 6px;background: rgba(42, 42, 42, 0.6);" @click.stop="clickTip"
    v-else>
    <div class="grade-tip small-tip" style="color: rgba(255, 255, 255, 0.85);">{{ tipTitle }}</div>
  </div>
  <a-modal v-model:visible="clickTipShow" :closable="false" title="" @ok="() => { }" :footer="null" :bodyStyle="{ padding: 0 }"
    style="font-size: 8px;" :maskClosable="false" :centered="true">
    <div class="flex flex-col items-center rounded-lg h-56 w-full justify-evenly text-sm text-white "
      ...
      <div>
        <Icon icon="-dengpao" style="font-size: 1.2em" class="mr-2"></Icon>提示
      </div>
      ...
      <div class="flex flex-row w-2/3  justify-between">
        <div class="rounded-lg w-28 h-10 flex justify-center items-center mt-4 pointer"
          style="background: rgba(42, 42, 42, 1);" @click="goGrade">了解更多</div>
        <div class="rounded-lg w-28 h-10 flex justify-center items-center mt-4 pointer"
          style="background: rgba(42, 42, 42, 1);" @click="closeTip">关闭</div>
      </div>
    </div>
  </a-modal>
</template>
