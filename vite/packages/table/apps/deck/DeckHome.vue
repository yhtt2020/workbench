<template>
  ...
  <template v-else>
    <div v-if="this.grids.length === 0" class=" box p-5 w-full item-content " style="max-height: 100%">
      <a-result class="w-full rounded-lg xt-text" style="background: none;width:100%"
                status="success" title="使用快捷指令"
  ...
  <div v-if="this.grids.length !== 0" class="flex flex-col h-full w-full">
    <div v-if="sharing"  >
      <div class="font">请选择您要分享的小组，勾选后，在底部选择需要分享的方式。</div>
    </div>

    <div class="flex-1 h-0" :class="{ sharing: sharing }">
      <vue-custom-scrollbar @contextmenu.stop="showMenu(-1, undefined, 'wrapper')" :settings="scrollbarSettings"
        ...
        <div style="width: auto;white-space: nowrap">
          <div @contextmenu.stop="showMenu(grid.id)" :style="{ width: getWidth(grid.cols) }" style="display: inline-block"
            ...
            <h3 class="pointer text-more xt-text pl-3" v-if="sharing">
              <a-checkbox v-model:checked="selectedGridIds[grid.id]" class="xt-text">{{ grid.title }}</a-checkbox>
            </h3>
            <template v-else>
              ...
              <span v-if="editing"><span style="margin-left: 0.8em"><left-square-outlined v-if="index !== 0"
                ...
                <span class="pl-5 xt-text"></span> {{ grid.title }}</span>
              </h3>
            ...
            <div>
              ...
              <vuuri v-show="grid.children.length !== 0 || editing" :style="{ width: getVuuriWidth(grid.cols) }"
                     :key="key"
                     :drag-enabled="editing" group-id="grid.id" :ref="'grid' + grid.id" style="min-height: 3em"
                     item-key="id"
                     class="grid s-bg" :get-item-width="getIconSize" :get-item-height="getIconSize"
    ...
    <div v-if="sharing" class="sharing-bottom-panel w-full p-2" >
      <a-row>
        <a-col :span="12">
          当前选中的组：
          <span class="xt-text-2" v-if="selectedGrids.length === 0">请选择希望分享的组!</span>
  ...
  <a-drawer title="设置分享信息" placement="right" :closable="true" v-model:visible="shareMenuComVisible"
            @close="onClose">
    <div class="line">
      分享标题：
  ...
  <a-drawer title="导入方案" placement="right" :closable="true" v-model:visible="importMenuJsonVisible"
            @close="onClose">
    <div class="line">
    ...
    <div class="line">
      <a-button :disabled="this.importJsonTxt === ''" class="mr-5" type="primary" @click="importCode"
                style="background: var(--primary-bg);color: var(--primary-text);">导入分享代码
      </a-button>
      <a-button @click="importFile" style="background: var(--primary-bg);color: var(--primary-text);">导入文件
      </a-button>
    </div>
