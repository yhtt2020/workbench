<template>
 <div style='width:400px;height:400px;background: #212121;border: 1px solid rgba(255,255,255,0.1);box-shadow: 0px 0px 10px 0px rgba(0,0,0,0.5);border-radius: 10px;top: 135px;' class="float-icon" id="selectIcon">
   ...
   <div class="action-search pl-3 pr-3 pt-3 flex" style="justify-content: space-between;">
    <a-input style="height:40px;background: #191919;
    border: 1px solid rgba(255,255,255,0.1);flex:1;width: 80%; border-radius: 10px;" placeholder="搜索" v-model:value="searchValue"
    @change="searchIcon"
    />
   ...
   <div v-if="searchValue" class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap" style="height: 290px;">
    <div v-for="(item,index) in searchIcon" :key="index" @click="onSelectIcon(index,'emoji',item.name)" class="flex justify-center items-center mt-2 ml-2 pointer" style="width: 40px;height:40px;border-radius: 10px;" :class="selectIcon == index ? 'sel-active':''">
      <a-tooltip>
       ...
       <a-avatar  :src="'https://a.apps.vip/icons/goodSelect/'+item.name+'.svg'" :alt="item.alias" width="32" height="32"></a-avatar>
      </a-tooltip>
    </div>
  </div>


   <div  class="flex pl-1 pr-1 overflow-hidden overflow-y-auto xt-scrollbar flex-wrap" style="height: 290px;">
...
import {goodIcon} from '../components/goods'
...
export default {
 ...
 methods: {
   ...
   //随机图标 
   onRandom(){
     let randomLength = this.emojisList.length
